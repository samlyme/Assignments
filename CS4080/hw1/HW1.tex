...
\begin{enumerate}
    \item {
        I picked Haskell. For the compiler, they actually use Haskell! The language 
        is implemented in itself. This is to be expected since Haskell is pretty 
        popular for compilers.
    }

    \item {
        Just-in-time compilation is extremely difficult to do well. This is because 
        JIT requires extremely deep knowledge of the language and implementation.
        Improperly implement JIT, even if correct, can actually end up costing 
        performance rather than gaining.
    }

    \item {
        Interpreted languages can be easier to develop with since you can type 
        straight into the REPL and see the effects. However for performance,
        compiled language has them beat. By allowing both an interpreted and 
        compiled runtime option, Lisp can get the best of both worlds.
    }
\end{enumerate}
\section*{Chapter 3}
...
class LinkedListStack {
    ...
    \item {
        ...
        Example in JS:
        \lstinputlisting[]{ch3/truthyness.js}

        Example in Python:
        \lstinputlisting[language=Python]{ch3/truthyness.py}
    }
...
\section*{Github Repo}
https://github.com/samlyme/Assignments/tree/main/hw1
