...
int getLine(Chunk* chunk, int offset) {
    ...
    \item {
        ...
        The implementation is relatively simple. We just check if the current 
        number of constants is less than 256. If this is the case, a normal 
        \verb|OP_CONSTANT| is written, otherwise, we issue \verb|OP_CONSTANT_LONG|.
...
void writeConstant(Chunk* chunk, Value value, int line) {
    ...
    if (constantIdx < 256) {
        ...
        The most obvious sacrifice is that it uses an extra 
        `instruction slot'. We only have 256 possible 
        instructions because we encode them as a byte.
        This also adds a layer of complexity.
    }
    ...
    \item {
        ...
        At a very high level, malloc and free use doubly linked 
        lists to track memory. When you request memory,
        malloc traverses the list and finds a block with 
        enough memory, which it returns to you. Calling 
        free takes your pointer and connects it to the previous 
        block of free memory.
    }
\end{enumerate}

\section*{Chapter 15}
https://github.com/samlyme/Assignments/tree/cs4080/ch15/challenge
...
\begin{enumerate}
    ...
    writeChunk(&chunk, OP_ADD, 5);
        ...
        I think that it actually makes sense to remove the \verb|OP_SUBTRACT|. 
        We just need to add a single \verb|OP_NEGATE| after values we intend to 
        subtract, then use the \verb|OP_ADD|. Removing \verb|OP_NEGATE| requires 
        us to push a zero onto the stack for all negative values, which can be 
        expensive.
    }
    ...
    \item {
        ...
        The first benefit is that our process only uses as much memory as it needs.
        At the OS level, dynamic allocation means that the process can grow/shrink.
        However, this will be much slower, as we are now at the mercy of the 
        memory allocator. Also, there is just a general overhead when a dynamic 
        array for the stack, since we need to track the capacity, and that introduces 
        an `if' statement, which can add up.
    }

    \item {
        Are there other instructions where you can do a similar optimization?

        Yes, we can do the same optimization for all binary operations, since we 
        don't actually need to pop `a' off the stack.
