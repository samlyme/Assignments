...
\noindent
Tools and resources used: Mainly Google, and Gemini when I got really stuck with Lisp.
\begin{enumerate}
  \item[Exercise 1]
  ...
  \item[Choice Exercise 4]
    ...
    {\color{green!50!black} Self-assessed score: 5/5}.
    {\color{magenta} Questions/Feedback: Correctness}
  \item[Choice Exercise 6]
    {\color{red} Points attempted: 10}.
    {\color{blue} Time spent: lost count, would guess around 3 hours? (just because of LISP)}.
    {\color{brown} Difficulty: 8/10}.
    {\color{green!50!black} Self-assessed score: 10/10}.
    {\color{magenta} Questions/Feedback: Correctness}
\end{enumerate}
\pagebreak
...
\section*{Choice Exercise 4 [5]}
\begin{enumerate}
    \item {
        [5] We sometimes describe groups by their presentation, \(\langle S \mid R \rangle\), where \(S\) is a set of generators, and \(R\) is a set of relations. Recall, we define the dihedral group of the \(n\)-gon as \[         D_n = \langle \underbrace{r, f}_S \mid \underbrace{r^n = f^2 = (rf)^2 = e}_R \rangle.       \] What this means is that every element of \(s \in D_n\) can be written as a word \[         s = g_1^{e_1}g_2^{e_2}\dots g_m^{e_m},       \] where \(g_i \in S = \{r, f\}\), \(e_i \in \mathbb{Z}\), and where two words are equal if we can modify one to the other using the relations in \(R\).

        \begin{enumerate}
            \item {
                Draw a Cayley diagram of the group with presentation \[             G = \langle r_1, r_2 \mid r_1^2 = r_2^2 = (r_1 r_2)^3 = e \rangle.           \] (Hint: \(|G| \leq 10\).)
                 
                 \includegraphics[width=0.7\linewidth]{cayley.jpg}
            }
            \item {
                Use the generators and relations to prove that \(r^{-3}frf^3 = r \in D_5\).
        
                In \(D_5\), \(r^5 = e\), \(r^{k}f = fr^{n-k}\), \(f^2 = e\), and \(r^k = r^{5 - k}\)
            ...
            \item {
                Prove that for all positive integers \(k\), \((r^kf)^2 = e \in D_n\).
        
                \begin{proof}
                    We begin by expanding the express to \(r^kfr^kf\). Then, 
                    we rewrite as \(f r^{n-k} r^k f\). Notice that \(r^{n-k} r^k = r^n = e\).
                    Thus, we simplify down to \(ff = e\).
                \end{proof}
            }

            \item {
                Write a group presentation of the cyclic group \(C_n\).

                The group presentation of \(C_n\) is
                \[C_n = \langle r \mid r^n = e \rangle .\]
            }
        \end{enumerate}
    }
...
\section*{Choice Exercise 6 [10]}
\begin{enumerate}
    \item {
        [3] Write a function that takes a word in \(S_n\) and outputs it in cycle notation. For instance, if the word is \(\pi = 156243 \in S_n\) then the input to your program would be [1,5,6,2,4,3], and since \(\pi = (254)(36)\) in cycle notation, the output would be [[254],[36]] or something equivalent.

        \lstinputlisting[lastline=17]{word2cycle.py}

        I was unfortunately not able to write this in Scheme LISP. I was 
        defeated, but I managed to solve the other way
    }

    \item {
        [2] Write a function that goes the other way: it takes in input of a permutation in cycle notation (e.g. [[254],[36]]) and outputs the corresponding permutation as a word (e.g. [1,5,6,2,4,3].)

        \lstinputlisting{cycle2word.scm}
    }

    \item {
        [2] Write a function that takes two permutations as words and composes them, working right-to-left as usual.
    

        \lstinputlisting[firstline=19, lastline=30]{word2cycle.py}
    }

    \item {
        [2] Write a function that takes two permutations in cycle notation and composes them, working right-to-left as usual.

        \lstinputlisting[firstline=32, lastline=51]{word2cycle.py}
    }

    \item {
        [1] Write two functions that compute the order of a permutation, one that works on words, and one that works on cycle notation.

        \lstinputlisting[firstline=52]{word2cycle.py}
